    ...
    for(size_t i = 0; i < vpKf.size();++i)
    {
        ...
        for(size_t y = 0; y< kf->im_.rows; y++)
          for(size_t x = 0; x< kf->im_.cols; x++){
            if(kf->SemiDensePointSets_.at<float>(y,3*x+2)>max_d) {
                max_d = kf->SemiDensePointSets_.at<float>(y,3*x+2);
            }
          }
          max_d = 2*max_d/3;*/
        ...
        for(size_t y = 0; y< kf->im_.rows; y++)
          for(size_t x = 0; x< kf->im_.cols; x++)
        {
          //if(kf->SemiDensePointSets_.at<float>(y,3*x+2) > max_d) continue;
          
          if(kf->GradImg.at<float>(y,x) < 17) continue;
          ...
          if(Pw[2]>0)
          {
            float b = kf->rgb_.at<uchar>(y,3*x) ;
            float g = kf->rgb_.at<uchar>(y,3*x+1) ;
            float r = kf->rgb_.at<uchar>(y,3*x+2) ;
            glColor3ub(r,g,b);
